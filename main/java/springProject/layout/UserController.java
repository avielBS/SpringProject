...
public class UserController {
	...
	@RequestMapping(path="myapp/users",
			method = RequestMethod.GET,
			consumes = MediaType.APPLICATION_JSON_VALUE,
			produces = MediaType.APPLICATION_JSON_VALUE)
	public UserBoundry[] newUser () {
		UserBoundry[] users = this.userService.getUsers().stream().map(UserBoundry::new)
				.collect(Collectors.toList()).toArray(new UserBoundry[0]);
		return users;
	}
	
	@RequestMapping(path="myapp/newuser",
	...
	@RequestMapping(path="myapp/update/{email}",
			method = RequestMethod.POST,
			consumes = MediaType.APPLICATION_JSON_VALUE,
			produces = MediaType.APPLICATION_JSON_VALUE)
